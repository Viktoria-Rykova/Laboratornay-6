...
namespace компилятор_2
{
    ...
    if (!CheckUnsavedChanges())
        ...
        {
            // Получаем активный RichTextBox на текущей вкладке
            ...
            // Создаем экземпляр LexicalAnalyzer
            LexicalAnalyzer analyzer = new LexicalAnalyzer();
            List<LexicalAnalyzer.MatchResult> results = new List<LexicalAnalyzer.MatchResult>();

            // Обрабатываем выбор в ComboBox
            if (comboBox1.SelectedItem == null)
            {
                MessageBox.Show("Сначала выберите тип задания из выпадающего списка.");
                return;
            }
            ...
            results = analyzer.FindMatches(inputText, selected);


            // Очищаем DataGridView
            dataGridView1.Rows.Clear();
            currentTextBox.Select(0, currentTextBox.Text.Length);
            currentTextBox.SelectionBackColor = Color.White; // Сброс фонового выделения
            ...
            {
                // Если найдены совпадения, отображаем их в DataGridView
                dataGridView1.Visible = true;
                label1.Visible = false;

                // Добавляем результаты в таблицу и выделяем найденные участки в RichTextBox
                foreach (var result in results)
                {
                    dataGridView1.Rows.Add(result.Value, result.Position, result.Message);

                    // Выделяем найденное в RichTextBox
                    currentTextBox.Select(result.Position, result.Value.Length);
                    currentTextBox.SelectionBackColor = Color.Yellow;
                }
